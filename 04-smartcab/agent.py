...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            # self.epsilon -= 0.05
            self.epsilon = 0.997**self.train_count
            # self.epsilon = 1.0/(self.train_count**2)
            # self.epsilon = math.exp(-0.01*self.train_count)
    ...
    def createQ(self, state):
        ...
        if self.learning and state not in self.Q:
            ...
            self.Q.setdefault(state, {action: 0.0 for action in self.valid_actions}) # reviwer advice
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon > random.random():
                ...
                #     [k for k, v in self.Q[state].items() if v == self.get_maxQ(self.state)])
                action = [k for k, v in self.Q[state].items() if v == self.get_maxQ(self.state)][0]
        else:
...
def run():
    ...
    sim = Simulator(env, update_delay=0.01, display=True,
